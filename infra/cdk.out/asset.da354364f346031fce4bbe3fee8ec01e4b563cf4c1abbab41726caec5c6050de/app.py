...
def list_tasks(user_pk: str) -> List[Dict[str, Any]]:
    # Query-ish via pk; in single-table design this is a Query, not Scan
    # But we don't have a SK prefix index; use begins_with filter
    resp_ = TABLE.query(
        KeyConditionExpression="pk = :pk AND begins_with(sk, :p)",
        ExpressionAttributeValues={":pk": user_pk, ":p": "TASK#"},
    )
